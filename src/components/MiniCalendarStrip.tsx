...
export default function MiniCalendarStrip({ days, onDayClick }: { days: DayData[]; onDayClick?: (d: string) => void }) {
  return (<div className="flex gap-1.5 sm:gap-2">
    {days.map((day, i) => {
      const isToday = i === days.length - 1; const d = new Date(day.date + "T00:00:00");
      return (<button key={day.date} onClick={() => onDayClick?.(day.date)}
        ...
        {day.salesMet && day.color !== "grey" && <span className="absolute -right-0.5 -top-0.5 text-[10px]">🔥</span>}
        ...
        {day.color !== "grey" ? (<div className="mt-1 space-y-0.5 text-center">
        ...
        </div>) : (<div className="mt-1.5"><div className="h-1.5 w-1.5 rounded-full bg-slate-600"/></div>)}
