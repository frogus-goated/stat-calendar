...
export default function ModelCalendarPage() {
  ...
  const [loading, setLoading] = useState(true); const [entryModal, setEntryModal] = useState<any>(null);
  ...
  function getCalendarDays(): (DayData | null)[] {
    ...
    for (let d = 1; d <= dim; d++) { const ds = `${yr}-${String(mo+1).padStart(2,"0")}-${String(d).padStart(2,"0")}`; const e = em[ds]; if (!e) cells.push({ date: ds, sales: 0, newSubs: 0, ltv: 0, color: "grey", ltvMet: false }); else { const c = computeDay(e.sales, e.newSubs, model.minSalesGoal, model.excellentSalesGoal, model.ltvGoal); cells.push({ date: ds, sales: e.sales, newSubs: e.newSubs, ltv: c.ltv, color: c.color, ltvMet: c.ltvMet, notes: e.notes, entryId: e.id }); } }
    return cells;
  }
  const openEntry = (date: string) => { if (!isAdmin || !model) return; const e = entries.find(x => x.date === date); setEntryModal({ isOpen: true, date, modelId: model.id, modelName: model.name, minSalesGoal: model.minSalesGoal, excellentSalesGoal: model.excellentSalesGoal, ltvGoal: model.ltvGoal, existingEntry: e ? { id: e.id, sales: e.sales, newSubs: e.newSubs, notes: e.notes } : null }); };
  const handleExportPDF = () => { if (!model) return; exportMonthlyCalendarPDF(model, yr, mo, entries, forecastMonth(entries, todayStr, model.minSalesGoal)); };
  ...
  const forecast = forecastMonth(entries, todayStr, model.minSalesGoal); const prevForecast = forecastMonth(prevEntries, prevMR.to, model.minSalesGoal);
  ...
  return (
    ...
    <div className="animate-fade-in">
      ...
      <div className="flex items-center justify-between"><div><h1 className="font-display text-2xl font-bold">{model.name}</h1><p className="text-sm text-[var(--text-muted)]">Min: ${model.minSalesGoal} · Excellent: ${model.excellentSalesGoal} · LTV: ${model.ltvGoal}</p>{!isAdmin && <p className="text-xs text-[var(--accent)]">👁 View only</p>}</div>{isAdmin && <button onClick={handleExportPDF} className="btn btn-ghost text-xs">Export PDF</button>}</div>
      ...
      <div className="mt-4"><ForecastCard forecast={forecast} prevForecast={prevForecast} title={model.name} ltvGoal={model.ltvGoal}/></div>
      ...
      <div className="overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--bg-card)]">
        ...
        <div className="grid grid-cols-7">
          {days.map((dd, i) => {
            ...
            return (<button key={dd.date} onClick={() => openEntry(dd.date)} className={`group relative min-h-[80px] border-b border-r border-[var(--border)] p-1.5 text-left transition-all hover:brightness-125 sm:min-h-[100px] sm:p-2 day-${dd.color} ${isT ? "ring-2 ring-inset ring-[var(--accent)]" : ""} ${!isAdmin ? "cursor-default" : ""}`}>
              {dd.ltvMet && dd.color !== "grey" && <span className="absolute right-1 top-1 text-[11px]">🔥</span>}
      ...
      <div className="mt-4 flex flex-wrap items-center gap-4 text-xs text-[var(--text-secondary)]">
        <span className="flex items-center gap-1.5"><span className="inline-block h-3 w-3 rounded-sm day-green"/> Excellent (≥ ${model.excellentSalesGoal})</span>
        <span className="flex items-center gap-1.5"><span className="inline-block h-3 w-3 rounded-sm day-orange"/> Minimum (≥ ${model.minSalesGoal})</span>
        <span className="flex items-center gap-1.5"><span className="inline-block h-3 w-3 rounded-sm day-red"/> Below min</span>
        <span className="flex items-center gap-1.5">🔥 LTV ≥ ${model.ltvGoal}</span>
      </div>
