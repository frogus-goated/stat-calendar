...
export default function Dashboard() {
  ...
  function getModelDayData(model: ModelType, range: string[] = last7): DayData[] {
    ...
    return range.map(date => { const e = em[date]; if (!e) return { date, sales: 0, newSubs: 0, ltv: 0, color: "grey" as DayColor, salesMet: false }; const c = computeDay(e.sales, e.newSubs, model.salesGoal, model.minLtvGoal, model.excellentLtvGoal); return { date, sales: e.sales, newSubs: e.newSubs, ltv: c.ltv, color: c.color, salesMet: c.salesMet, notes: e.notes, entryId: e.id }; });
  }
  function getModelForecast(model: ModelType, which: "current" | "prev" = "current"): Forecast {
    ...
    return forecastMonth(entries, which === "current" ? todayStr : prevMonthRange.to, model.salesGoal);
  }
  function getTotalsForecast(which: "current" | "prev" = "current"): Forecast {
    ...
    return forecastMonth(agg, which === "current" ? todayStr : prevMonthRange.to, filteredModels.reduce((s, m) => s + m.salesGoal, 0));
  }
  function getTotalsDayData(): DayData[] {
    if (!totals) return last7.map(d => ({ date: d, sales: 0, newSubs: 0, ltv: 0, color: "grey" as DayColor, salesMet: false }));
    return totals.days.map(d => ({ date: d.date, sales: d.totalSales, newSubs: d.totalNewSubs, ltv: d.totalLtv, color: d.color as DayColor, salesMet: d.salesMet }));
  }
  ...
  function openDayModal(modelId: string, date: string) {
    const model = models.find(m => m.id === modelId); if (!model) return;
    const existing = (modelEntries[modelId] || []).find(e => e.date === date);
    if (isAdmin) {
      setEntryModal({ isOpen: true, date, modelId, modelName: model.name, salesGoal: model.salesGoal, minLtvGoal: model.minLtvGoal, excellentLtvGoal: model.excellentLtvGoal, existingEntry: existing ? { id: existing.id, sales: existing.sales, newSubs: existing.newSubs, notes: existing.notes } : null });
    } else {
      const dayData = getModelDayData(model, [date])[0];
      setViewModal({ isOpen: true, day: dayData, modelName: model.name, salesGoal: model.salesGoal, minLtvGoal: model.minLtvGoal, excellentLtvGoal: model.excellentLtvGoal });
    }
  }
  ...
  // Global green streak for toast
  const allGreenStreak = (() => { const totalsDays = getTotalsDayData(); return calculateGreenStreak(totalsDays.map(d => ({ date: d.date, color: d.color })), todayStr); })();
  ...
  const prevTotalsDayData: DayData[] | null = totals ? prev7.map(date => { let ts = 0, tsubs = 0; for (const m of filteredModels) { const e = (modelEntries[m.id] || []).find(x => x.date === date); if (e) { ts += e.sales; tsubs += e.newSubs; } } return { date, sales: ts, newSubs: tsubs, ltv: tsubs > 0 ? ts / tsubs : 0, color: "grey" as DayColor, salesMet: false }; }) : null;
  const totalsForecast = getTotalsForecast(); const prevTotalsForecast = getTotalsForecast("prev");
  const totalsLtvGoal = totals?.totalsExcellentLtvGoal ?? 50;

  return (<><Nav />
  <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6"><div className="animate-fade-in space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div><h1 className="font-display text-2xl font-bold tracking-tight sm:text-3xl">Dashboard</h1>
      ...
      {isAdmin && (<div className="flex flex-wrap gap-2"><button onClick={handleExportPDF} className="btn btn-ghost text-xs">Export PDF</button><button onClick={() => setModelModal({ isOpen: true, editModel: null })} className="btn btn-primary"><span className="text-lg leading-none">+</span> Add Model</button></div>)}
    </div>
    {isAdmin && (<div className="flex flex-wrap items-center gap-2 rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-2">
    ...
    {filteredModels.length === 0 ? (<div className="rounded-2xl border border-dashed border-[var(--border)] bg-[var(--bg-card)] p-12 text-center"><p className="text-[var(--text-muted)]">{isAdmin ? "No models yet. Add one above." : "No models assigned to you yet. Ask your admin."}</p></div>) : (
      <div className="space-y-4">
        {filteredModels.map(model => {
          ...
          return (<div key={model.id} className="rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] p-4 sm:p-5">
            <div className="mb-3 flex items-center justify-between">
              <div><div className="flex items-center gap-2"><h3 className="font-display text-base font-semibold">{model.name}</h3>{model.group && <span className="rounded-full bg-[var(--accent)]/10 px-2 py-0.5 text-[10px] font-medium text-[var(--accent)]">{model.group.name}</span>}</div><p className="text-xs text-[var(--text-muted)]">Sales: ${model.salesGoal} 🔥 · Min LTV: ${model.minLtvGoal} · Excellent: ${model.excellentLtvGoal}</p></div>
              <div className="flex items-center gap-2">
                {isAdmin && <button onClick={() => setModelModal({ isOpen: true, editModel: { id: model.id, name: model.name, salesGoal: model.salesGoal, minLtvGoal: model.minLtvGoal, excellentLtvGoal: model.excellentLtvGoal, groupId: model.groupId } })} className="rounded-lg p-1.5 text-[var(--text-muted)] hover:bg-[var(--bg-hover)]" title="Edit"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>}
                <a href={`/model/${model.id}`} className="btn btn-ghost text-xs">Calendar →</a>
              </div>
            </div>
            <MiniCalendarStrip days={dayData} onDayClick={date => openDayModal(model.id, date)} />
            <div className="mt-3"><QuickStats days={dayData} prevDays={prevDayData} /></div>
            <div className="mt-3"><ForecastCard forecast={forecast} prevForecast={prevForecast} title={model.name} ltvGoal={model.excellentLtvGoal} /></div>
    ...
    <StreakToast greenStreak={allGreenStreak} />
  </div></main></>);
}
