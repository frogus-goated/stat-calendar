...
async function checkAccess(userId: string, role: string, modelId: string): Promise<boolean> {
  ...
  const access = await prisma.userModelAccess.findUnique({ where: { userId_modelId: { userId, modelId } } });
  return !!access;
}
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (b.minSalesGoal !== undefined) d.minSalesGoal = Number(b.minSalesGoal);
  if (b.excellentSalesGoal !== undefined) d.excellentSalesGoal = Number(b.excellentSalesGoal);
  if (b.ltvGoal !== undefined) d.ltvGoal = Number(b.ltvGoal);
