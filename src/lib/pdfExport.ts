...
export function exportWeeklyPDF(
  ...
  const tRows = totalsDayData.map(d => ({ d: d.date.substring(5), s: `$${d.sales.toFixed(0)}`, sb: String(d.newSubs), l: `$${d.ltv.toFixed(2)}`, c: d.color, _c: cMap[d.color]||cMap.grey, fire: d.ltvMet?"🔥":"" }));
  autoTable(doc, {
    startY: 38, head: [["Date","Sales","Subs","LTV","Status","KPI"]],
  ...
  for (const { model, dayData, forecast } of models) {
    ...
    doc.text(`Min: $${model.minSalesGoal} | Excellent: $${model.excellentSalesGoal} | LTV Goal: $${model.ltvGoal}`, 14, y+4); doc.setTextColor(0); y+=7;
    const mR = dayData.map(d => ({ d:d.date.substring(5), s:`$${d.sales.toFixed(0)}`, sb:String(d.newSubs), l:`$${d.ltv.toFixed(2)}`, c:d.color, _c:cMap[d.color]||cMap.grey, fire:d.ltvMet?"🔥":"" }));
    autoTable(doc, { startY: y, head: [["Date","Sales","Subs","LTV","Status","KPI"]], body: mR.map(r => [r.d,r.s,r.sb,r.l,r.c.toUpperCase(),r.fire]), theme: "grid", headStyles: { fillColor:[99,102,241], textColor:255, fontSize:8 }, bodyStyles: { fontSize:8 }, didParseCell(data:any) { if(data.section==="body"&&data.column.index===4) { const r=mR[data.row.index]; if(r){data.cell.styles.textColor=r._c; data.cell.styles.fontStyle="bold";} } }, margin: { left:14, right:14 } });
...
export function exportMonthlyCalendarPDF(model: ModelType, year: number, month: number, entries: EntryType[], forecast: Forecast) {
  ...
  doc.text(`Min: $${model.minSalesGoal} | Excellent: $${model.excellentSalesGoal} | LTV: $${model.ltvGoal}`, 14, 24); doc.setTextColor(0);
  ...
  for (let d=1;d<=dim;d++) {
    ...
    const e = em[ds];
    if (e) { const c = computeDay(e.sales,e.newSubs,model.minSalesGoal,model.excellentSalesGoal,model.ltvGoal); cur.push(`${d}\n$${e.sales.toFixed(0)} | ${e.newSubs}s\n[${c.color.toUpperCase()}]${c.ltvMet?" 🔥":""}`); }
    else cur.push(`${d}\n—`);
